...
def _send_otp_via_supabase_rest(
    email: str,
    full_name: str,
    preferred_lang: str,
) -> dict:
    """
    Send OTP via Supabase Auth REST API directly (HTTPS port 443).
    Avoids the Supabase Python Auth client, which can hit DNS issues
    on some Windows network configurations.
    """
    from app.core.config import settings
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info("OTP sent via REST", email=email)
        ...
        logger.error("OTP send failed", status=response.status_code, error=error_msg)
        raise ValidationException(error_msg)

    except ValidationException:
        raise
    except httpx.TimeoutException:
        logger.error("OTP send timed out", email=email)
        raise ValidationException("OTP request timed out. Please try again.")
    except Exception as e:
        logger.error("OTP send error", error=str(e), email=email)
...
class AuthService:
    ...
    async def send_otp(self, data: SendOtpRequest) -> dict:
        """
        Send OTP via Supabase Auth REST API directly.
        Uses HTTPS (port 443) instead of the Supabase Python Auth client.
        """
        return _send_otp_via_supabase_rest(
            email=data.email,
